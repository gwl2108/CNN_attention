...
#SET VARIABLES HERE:
imtype=1; #images: 1=merge, 2=array, 3=reg-test 
...
bsize=15*imperim;  Ncats=20; #np.length(Cats)
impercat=np.floor(bsize/19.0); leftov=bsize%19
...
if __name__ == '__main__':
    ...
    savstr='ObjectAttn'+'_a'+appwith+'_'+str(descat)+'c'+str(lyr)+'l_'+'a'+str(attype)+'bd'+str(bd)+'_im'+str(imtype)+'.npz'
    ...
    tp_batch=np.zeros((15*imperim,224,224,3)); tplabs=np.ones((bsize,1))
    tn_batch=np.zeros((15*imperim,224,224,3)); tnlabs=np.zeros((bsize,1))
    TPscore=np.zeros((len(astrgs))); TNscore=np.zeros((len(astrgs))); #FN=np.zeros((len(str_vec),xval)); TN=np.zeros((len(str_vec),xval))

    if rec_activity:
        tp_resps1=np.ones((2,64,Ncats,len(astrgs))); tp_resps2=np.ones((2,128,Ncats,len(astrgs)));  tp_resps3=np.ones((3,256,Ncats,len(astrgs)));
        tp_resps4=np.ones((3,512,Ncats,len(astrgs))); tp_resps5=np.ones((3,512,Ncats,len(astrgs)));
        tn_resps1=np.ones((2,64,Ncats,len(astrgs))); tn_resps2=np.ones((2,128,Ncats,len(astrgs)));  tn_resps3=np.ones((3,256,Ncats,len(astrgs)));
        tn_resps4=np.ones((3,512,Ncats,len(astrgs))); tn_resps5=np.ones((3,512,Ncats,len(astrgs)));
	...
	if imtype==3:
		...
		imspercats=np.ones((19))*impercat; imspercats[np.random.choice(19,leftov)]+=1
	...
	for astrg in astrgs: 
	    ...
	    TPscore[ai]=np.sum(tp_score==tplabs,axis=0)/(bsize*1.0); #tp_score #catSV.score(tp_resp,testlabels1)
	    TNscore[ai]=np.sum(tn_score==tnlabs,axis=0)/(bsize*1.0);  #tn_score #catSV.score(tn_resp,testlabels0)
            ...
            if rec_activity: 
                ...
                tp_resps3[2,:,ci,ai]=np.mean(resp_list[6],axis=0); tp_resps4[0,:,ci,ai]=np.mean(resp_list[7],axis=0); 
                ...
                tn_resps3[2,:,ci,ai]=np.mean(resp_list[6],axis=0); tn_resps4[0,:,ci,ai]=np.mean(resp_list[7],axis=0); 
                tn_resps4[1,:,ci,ai]=np.mean(resp_list[8],axis=0); tn_resps4[2,:,ci,ai]=np.mean(resp_list[9],axis=0);
                tn_resps5[0,:,ci,ai]=np.mean(resp_list[10],axis=0); tn_resps5[1,:,ci,ai]=np.mean(resp_list[11],axis=0);
                tn_resps5[2,:,ci,ai]=np.mean(resp_list[12],axis=0);

     	         
    np.savez(save_path+'/'+savstr,TPscore,TNscore,astrgs)

    if rec_activity:
        alltp=[tp_resps1, tp_resps2,tp_resps3,tp_resps4,tp_resps5]
        np.savez(save_path+'/'+savstr+'TP.npz',alltp, astrgs)
        alltn=[tn_resps1, tn_resps2,tn_resps3,tn_resps4,tn_resps5]
        np.savez(save_path+'/'+savstr+'TN.npz',alltn, astrgs)
